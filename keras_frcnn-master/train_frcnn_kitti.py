...
c = tf.ConfigProto()
c.gpu_options.per_process_gpu_memory_fraction = 0.9 # 每个GPU现存上届控制在60%以内
...
def train_kitti():
    ...
    cfg.model_path = './model/superanov_nov_vgg.hdf5'
    cfg.simple_label_file = './data/list_split.csv'
    all_images, classes_count, class_mapping = get_data(cfg.simple_label_file, 'F:\\keras_frcnn-master\\data\\', '_a.jpg')
    ...
    rpn = nn.rpn(shared_layers, num_anchors)        # return layer : regr, linener, base
    ...
    for epoch_num in range(num_epochs):
        ...
        while True:
            try:
                ...
                X, Y, img_data = next(data_gen_train)   ## X, Y, sample weight
                print(X.shape, img_data)

                loss_rpn = model_rpn.train_on_batch(X, Y)

                print("train rpn ok")

                P_rpn = model_rpn.predict_on_batch(X)               ## P_rpn 为 regr 与 liner 的预测值
